...
def fmatrix(xy1, xy2):
	...
	Fp, ier = opt.leastsq(
        lambda params: fundamental_matrix_error(
            fmatrix_from_params(params),
            PZPT1,
            PZPT2,
            xy1_inliers,
            xy2_inliers
        ),
        Fp0, xtol = 1e-6, ftol = 1e-6, maxfev = 15000)
...
def fundamental_matrix_error(Fhat, PZPT1, PZPT2, xy1, xy2):
	...
	denom = sum(xy2_fit * dot(xy2_fit, PZPT1.T), axis = 1) \
    + sum(xy1_fit * dot(xy1_fit, PZPT2.T), axis = 1)
